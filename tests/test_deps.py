...
class TestDeps(TestCase):
    def test_effkey(self):
        self.assertEqual(
            effkey({"role": "a", "src": "b"}), "role", "EffKey role before src"
        )
...
""",
        ...
        self.assertEqual(
            o[0]["name"], "test-role-1", "simple extracted role name (role)"
        )
        self.assertEqual(
            o[1]["name"], "test-role-2", "simple extracted role name (src)"
        )
        self.assertEqual(
            o[2]["name"], "test-role-3", "simple extracted role name (name)"
        )
